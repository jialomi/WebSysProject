    ...
    <div class="flex-grow-1 bg-light">
        ...
        <main class="p-4">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
                <div class="d-flex flex-wrap gap-1" role="group" aria-label="Filter bookings by status">
                    <?php
                    $tabLabels = ['all' => 'All', 'pending' => 'Pending', 'confirmed' => 'Confirmed', 'cancelled' => 'Cancelled'];
                    foreach ($tabLabels as $val => $label):
                        $active = $filterStatus === $val ? 'btn-dark' : 'btn-outline-dark';
                    ?>
                        <a href="/admin/manage-bookings.php?status=<?= $val ?><?= $searchQuery ? '&q=' . urlencode($searchQuery) : '' ?>" class="btn btn-sm <?= $active ?>">
                            <?= htmlspecialchars($label) ?> <span class="badge <?= $filterStatus === $val ? 'bg-warning text-dark' : 'bg-secondary' ?> ms-1"><?= $counts[$val] ?></span>
                        </a>
                    <?php endforeach; ?>
                </div>
                ...
                <form action="/admin/manage-bookings.php" method="GET" class="d-flex gap-2">
                    ...
                    <label for="bookingSearch" class="visually-hidden">Search bookings</label>
                    <input type="search" id="bookingSearch" class="form-control form-control-sm" name="q" placeholder="Search customer, car…" value="<?= htmlspecialchars($searchQuery) ?>">
                    <button type="submit" class="btn btn-sm btn-warning" aria-label="Submit search">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="fw-semibold text-nowrap">
                                        <a href="/admin/manage-bookings.php?edit=<?= (int)$b['id'] ?>" class="text-dark text-decoration-none">#<?= (int)$b['id'] ?></a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="small fw-semibold"><?= htmlspecialchars($b['brand'] . ' ' . $b['model']) ?></div>
                                        <div class="text-muted" style="font-size:0.75rem;">SGD <?= number_format((float)$b['daily_rate'],2) ?>/day</div>
                                    </td>
                                    <td class="small">
                                    ...
                                    <td class="small text-muted"><?= htmlspecialchars($b['pickup_location']) ?></td>
                                    <td>
                                        <div class="fw-semibold">SGD <?= number_format((float)$b['total_cost'],2) ?></div>
                                        <?php if ($b['promo_code']): ?>
                                        <div class="text-success" style="font-size:0.72rem;"><?= htmlspecialchars($b['promo_code']) ?> applied</div>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <span class="badge status-<?= htmlspecialchars($b['status']) ?> px-2 py-1 text-capitalize"><?= htmlspecialchars($b['status']) ?></span>
                                    </td>
                                    <td>
                                        <div class="d-flex gap-1 flex-wrap">
                                            ...
                                            <form method="POST" action="/admin/manage-bookings.php" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-xs btn-success btn-sm" aria-label="Confirm booking <?= (int)$b['id'] ?>" title="Confirm booking">
                                            ...
                                            <form method="POST" action="/admin/manage-bookings.php" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-xs btn-danger btn-sm btn-cancel-booking" aria-label="Cancel booking <?= (int)$b['id'] ?>" title="Cancel booking">
                                                    <i class="bi bi-x-lg" aria-hidden="true"></i>
                                                </button>
                                            </form>
                                            <?php endif; ?>
                                            <button class="btn btn-xs btn-outline-secondary btn-sm" aria-label="View details for booking <?= (int)$b['id'] ?>" title="View details" data-bs-toggle="modal" data-bs-target="#detailModal<?= (int)$b['id'] ?>">
                                    ...
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h2 class="h5 modal-title" id="detailModalLabel<?= (int)$b['id'] ?>">Booking #<?= (int)$b['id'] ?> Details</h2>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <dl class="row small mb-0">
                                                    <dt class="col-sm-4">Customer</dt><dd class="col-sm-8"><?= htmlspecialchars($b['customer_name']) ?><br><span class="text-muted"><?= htmlspecialchars($b['customer_email']) ?></span></dd>
                                                    <dt class="col-sm-4">Car</dt><dd class="col-sm-8"><?= htmlspecialchars($b['brand'] . ' ' . $b['model']) ?></dd>
                                                    <dt class="col-sm-4">Pickup</dt><dd class="col-sm-8"><?= htmlspecialchars($b['pickup_location']) ?></dd>
                                                    <dt class="col-sm-4">Dates</dt><dd class="col-sm-8"><?= htmlspecialchars(date('d M Y', strtotime($b['start_date']))) ?> → <?= htmlspecialchars(date('d M Y', strtotime($b['end_date']))) ?></dd>
                                                    <dt class="col-sm-4">Total</dt><dd class="col-sm-8 fw-bold">SGD <?= number_format((float)$b['total_cost'],2) ?></dd>
                                                    <?php if ($b['discount_amount'] > 0): ?>
                                                    <dt class="col-sm-4">Discount</dt><dd class="col-sm-8 text-success">- SGD <?= number_format((float)$b['discount_amount'],2) ?> (<?= htmlspecialchars($b['promo_code']) ?>)</dd>
                                                    <?php endif; ?>
                                                    <dt class="col-sm-4">Status</dt><dd class="col-sm-8 text-capitalize"><?= htmlspecialchars($b['status']) ?></dd>
                                                    <dt class="col-sm-4">Created</dt><dd class="col-sm-8"><?= htmlspecialchars(date('d M Y H:i', strtotime($b['created_at']))) ?></dd>
                                                    <?php if ($b['notes']): ?>
                                                    <dt class="col-sm-4">Notes</dt><dd class="col-sm-8"><?= htmlspecialchars($b['notes']) ?></dd>
                                                    <?php endif; ?>
                                                </dl>
                                            </div>
